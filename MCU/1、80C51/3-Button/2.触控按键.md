...
在`Keil`创建项目时，选择芯片为`AT89C52`，但**`IC`厂商为`STC`**，并非`AT`！因此`#include <reg52.h>`**没有`P4 IO`端口定义**！！

因此，参考`datasheet`手动定义`P4`：

<img src="https://raw.githubusercontent.com/root-bine/image/main/Typora-image/Button06.png" alt="image-20250911165057947" style="zoom:40%;" />
...
`LED`按**默认方向**依次点亮，当**按下`Touch Button`**，`LED`流水灯方向改变，松开则恢复默认方向。
...
void open_led(unsigned int num); // 调试时, 须补全

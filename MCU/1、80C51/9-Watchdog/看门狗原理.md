...
✅<span style="color:red"><u>看门狗机制</u></span>：
...
3. **指定时间**内：
   ...
   | 1、不喂狗   ->     溢出，发出<span style="color:red">复位信号</span> | 2、重新喂狗   ->   程序正常执行 |
   | :----------------------------------------------------------: | :-----------------------------: |
...
$$
`datasheet`也直接给出**不同频率单片机**的**参数表格**：
...
选择`WDT Overflow Time = 2.2755 s`，因此：`PS2 = 1`、`PS1 = 0`、`PS0 = 1`  
...
`while(1) {delay_ms(3000);}`执行延时：`3 s > 2.2755 s`，触发`Watchdog Reset`：

...
| 1、外部连接按键`KEY2` | 2、引脚复用为`ADC123_IN12` |
| :-------------------: | :------------------------: |

因此，设置该引脚功能：`GPIO_EXTI2`。
...
> 在中断操作中，<span style="color:red">不建议使用延时函数</span>，因为**延时函数也依赖于中断** 

❗`HAL_Delay`函数，依赖中断：`Time base: System tick timer`，因此需要修改`Preemption Priority`，否则会卡死。

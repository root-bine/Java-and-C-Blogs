...
```c
"printf函数头文件 + gxht30地址":
#include <stdio.h>
#define WRITE_ADDRESS (0x44<<1) + 0
#define READ_ADDRESS  (0x44<<1) + 1
"CRC校验 + 数据转换 + printf函数重定向":
...
"模式命令":
uint8_t cmd_buffer[2] = {0x21, 0x30};
"各类参数":
uint8_t dat[6];
float temperature = 0;
float humidity   = 0;
"初始化gxht30":
HAL_I2C_Master_Transmit(&hi2c2, WRITE_ADDRESS, cmd_buffer, sizeof(cmd_buffer), 0xFF);
"读取数据 + 解析数据并打印":
while (1) {
	if(HAL_I2C_Master_Receive(&hi2c2, READ_ADDRESS, dat, sizeof(dat), 0xFF) == HAL_OK){
        if(GXHT3L_Dat_To_Float(dat, &temperature, &humidity) == 0) {
...
"开启周期测量 + 读取数据 + 解析数据并打印":
while (1) {
	HAL_I2C_Master_Transmit(&hi2c2, WRITE_ADDRESS, cmd_start, sizeof(cmd_start), 0xFF);
	if(HAL_I2C_Master_Receive(&hi2c2, READ_ADDRESS, dat, sizeof(dat), 0xFF) == HAL_OK){
        if(GXHT3L_Dat_To_Float(dat, &temperature, &humidity) == 0) {
            printf("temperature = %f, humidity = %f\n", temperature, humidity);
		}

...
- **Mode_0**主要用于<span style="color:green">并行口扩展</span>；

- <span style="color:red">**Mode_1**是最常用的工作模式</span>；

- **Mode_2**和**Mode_3**，**多出的第9位**可作**校验位**或**地址识别位**；
...
在`MCU`中使用`printf`函数，需要引入头文件：`#include <stdio.h>`，<span style="color:red">重写printf调用的putchar函数，重定向到串口输出</span>：

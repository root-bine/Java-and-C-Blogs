...
定时器有`Timer0`、`Timer1`，运行模式如下：

|                   Mode_Name                   |                  Explanation                   |
| :-------------------------------------------: | :--------------------------------------------: |
|                    Mode_0                     |                **13位**`Timer`                 |
| Mode_1（<span style="color:red">常用</span>） | <span style="color:red">**16位**`Timer`</span> |
...
> STM32F10xxx参考手册_ch.pdf：6.2 时钟 -> 图8 时钟树

`STM32`定时器有**4种时钟源选择**：

|  时钟源   |                           Explain                            |
| :-------: | :----------------------------------------------------------: |
| ①内部时钟 | `STM32F103`定时器的**时钟源**：<br>`TIM1`、`TIM8`：来自<span style="color:red">**APB2总线**上的`TIMxCLK`</span><br>`TIM2 ~ TIM7`：来自<span style="color:red">**APB1总线**上的`TIMxCLK`</span> |
| ②外部时钟 |             通过`TIMx_ETR`，接收**外部触发输入**             |
| ③内部触发 | 1. 通过`ITRx (x = 0, 1, 2, 3)`设置<br>2. 将任意`Timer`设置为**预分频器** |

...
`UART` (`Universal Asynchronous Receiver Transmitter`，**通用异步收发器**) ，可实现单片机的**串口通信**。
...
<img src="https://raw.githubusercontent.com/root-bine/image/main/Typora-image/UART02.png" alt="image-20251007105329751" style="zoom: 50%;" />

串行与并行对比：

- 串行通信**传输线少**，**长距离传送成本低**，**抗干扰能力强**；
- 并行通信**数据传输简单**，**传输速度快**；



## 2、<span style="color:brown">通信说明：</span>

|                   分类                    |                         描述                         |
| :---------------------------------------: | :--------------------------------------------------: |
|                 单工通信                  |                数据只能**单方向传输**                |
|                半双工通信                 | 通信双方**交替进行双向数据传输**，但**不能同时进行** |
...
✅**UART & USART**是<span style="color:red">**全双工**</span>传输方式，**I2C**是半双工传输方式。



## 3、<span style="color:brown">数据帧：</span>
...
空闲位**不是串口报文的数据**，是发送完一组报文后，**总线会自动将电平拉高**，产生`1 / 2 bit`逻辑`1`的**空闲位**。
...
`USART & UART`的数据发送按照<span style="color:red">`LSB-First`收发</span>：

<img src="https://raw.githubusercontent.com/root-bine/image/main/Typora-image/STM32_UART03.png" alt="image-20251121181730713" style="zoom:65%;" />

...
| 通信拓扑 | <span style="color:red">异步的 / 同步的、**点对点**总线</span> | <span style="color:red">同步的、**多主从**总线</span> |
...
板卡使用`STC89C52RC/RD+` ，在板卡集成一个**温湿度传感器**，采用**`I2C`通信模式**，`GPIO`端口：`P0.1`、`P0.2`。



## 3、<span style="color:brown">I2C通信：</span>
...
✅在`Master-Slave`通信中，<u>每个`Slave`</u>都有<span style="color:red">**唯一地址**</span>（通常<span style="color:red">`7 bit`</span>）。

...
<img src="https://raw.githubusercontent.com/root-bine/image/main/Typora-image/External_Interrupt07.png" alt="image-20250912183733783" style="zoom: 60%;" />

如图为**默认优先级**：`INT0`最高，`INT3`最低。

**1.2、IP / IPH寄存器：**

**`STC89C51RC/RD+`系列单片机**，`IP`寄存器可以**位操作**，`IPH`只能**寄存器操作**，详情如下：
...
<span style="color:red">**优先级可以不设置**</span>：

| 默认优先级是`0` | `Reset`后，`IPH`和`IP`的默认值变为`0` |
| --------------- | ------------------------------------- |
